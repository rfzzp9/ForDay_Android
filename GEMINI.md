# Forday Android - Gemini Entry

이 파일은 Gemini용 진입 라우터입니다. 프로젝트의 공통 지식은 `.agents/docs/` 아래에 둔다.
...
- 위치를 찾을 때는 `.agents/docs/llm-wiki/`를 먼저 본다.
- 코드 모양을 맞출 때는 `.agents/docs/conventions/`를 본다.
- 작업 순서를 정할 때는 `.agents/docs/workflows/`를 본다.
- `_workspace/`는 과거 리포트와 임시 산출물로만 사용한다.
